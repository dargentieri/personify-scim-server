...
public class SchemaReader {
	...
	public SchemaResourceType getSchemaResourceTypeByEndpoint(String endpoint) {
		for ( SchemaResourceType srt : schemaResourceTypeMap.values()) {
			if ( !endpoint.startsWith(StringUtils.SLASH)) {
				endpoint = StringUtils.SLASH + endpoint;
			}
			if ( srt.getEndpoint().equalsIgnoreCase(endpoint)) {
				return srt;
			}
		}
		return null;
