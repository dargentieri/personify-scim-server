...
public class DiscoveryController extends Controller {
	...
	@GetMapping(path = "/scim/v2/ResourceTypes/{param}", produces = { "application/scim+json", "application/json" })
	public ResponseEntity<?>  getResourceTypeUser(@PathVariable String param) {
		long start = System.currentTimeMillis();
		ResponseEntity<?> result = null;
		try {
			if (resourceTypes == null) {
			...
			{
				case "user":
					result = new ResponseEntity<Object>(resourceTypes.get(0), HttpStatus.OK);
				break;
				case "group":
					result = new ResponseEntity<Object>(resourceTypes.get(1), HttpStatus.OK);
				break;
				default:
	...
	@GetMapping(path = "/scim/v2/Schemas/{param}", produces = { "application/scim+json", "application/json" })
	public ResponseEntity<?> getSchemaUser(@PathVariable String param,HttpServletRequest request, HttpServletResponse response)
	{
		...
		try {
			...
			switch(param.toLowerCase())
			{
				case "urn:ietf:params:scim:schemas:core:2.0:user":
					result = new ResponseEntity<Object>(schemas.get(0), HttpStatus.OK);
				break;
				case "urn:ietf:params:scim:schemas:core:2.0:group":
					result = new ResponseEntity<Object>(schemas.get(1), HttpStatus.OK);
				break;
				default:
					result = new ResponseEntity<String>("", HttpStatus.NOT_FOUND);
				break;
