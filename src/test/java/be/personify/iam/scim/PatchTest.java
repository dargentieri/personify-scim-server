...
public class PatchTest {
	...
	@Test
	public void testGetPathSix() {
		Schema schema = schemaReader.getSchemaByName(Constants.RESOURCE_TYPE_USER);
		Map<String,Object> entity = new HashMap<String,Object>();
			
			
		Map<String,String> existingMail  = new HashMap<>();
		existingMail.put("type", "work");
		existingMail.put("value", "tobereplaced");
		List mails = new ArrayList<>();
		mails.add(existingMail);
		entity.put("emails", mails);
			
		patchUtils.patchEntity(entity, PatchOperation.replace, "emails[type eq \"work\"].value", "updatedEmail@microsoft.com", schema );
			
		logger.info("entity {}", entity);
		
		List p = (List)entity.get("emails");
					
		Assert.isTrue( p.size() == 1 , "has to have one mail");
		
		Map m = (Map)p.get(0);
		Assert.isTrue( m.get("value").equals("updatedEmail@microsoft.com") , "mail has to be overwritten to updatedEmail@microsoft.com");
	...
	@Test
	public void testGetPathSeven() {
		Schema schema = schemaReader.getSchemaByName(Constants.RESOURCE_TYPE_USER);
		Map<String,Object> entity = new HashMap<String,Object>();
			
			
		Map<String,String> existingMailOne  = new HashMap<>();
		existingMailOne.put("type", "work");
		existingMailOne.put("value", "tobereplaced");
		
		Map<String,String> existingMailTwo  = new HashMap<>();
		existingMailTwo.put("type", "other");
		existingMailTwo.put("value", "tonotbereplaced");
		
		List mails = new ArrayList<>();
		mails.add(existingMailOne);
		mails.add(existingMailTwo);
		entity.put("emails", mails);
		...
		Assert.isTrue( p.size() == 2 , "has to have two mail");
		...
		m = (Map)p.get(1);
		Assert.isTrue( m.get("value").equals("tonotbereplaced") , "mail has to not be overwritten tonotbereplaced");
